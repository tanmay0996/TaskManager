...
- `GET /` → `{ "message": "Task Manager API running" }`
...
- **Responses**:
  - `201 Created`
    ...
    ```
  - `400 Bad Request` – missing fields or username already taken
    ```json
    {
...
- For REST client tools (Postman, Insomnia, etc.), remember:
  - Register → Login → copy `token` → set `Authorization: Bearer <token>` for task routes.
